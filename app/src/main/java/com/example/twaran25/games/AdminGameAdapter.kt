...
class AdminGameAdapter(
    private val fullGameList: List<Game>,
    private val onSportClick: (String) -> Unit
) : RecyclerView.Adapter<AdminGameAdapter.AdminGameViewHolder>() {

    private var filteredGameList: MutableList<Game> = fullGameList.toMutableList() // List that updates dynamically

    class AdminGameViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AdminGameViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.games_design_card, parent, false)
        return AdminGameViewHolder(view)
    }

    override fun onBindViewHolder(holder: AdminGameViewHolder, position: Int) {
        ...
        holder.arrowButton.setOnClickListener {
            ...
            val intent = Intent(context, AdminMatchDetails::class.java) // Replace with your target activity
